...
### Answer
  1. If we lower the classification threshold to 20%, there will be more observations classified as 1 because more observations will meet the threshold and, consequently there will be more observations “correctly classified” as 1 → sensitivity improves. But this comes at the expense of correct negative predictions because now fewer observations will be correctly classified as negative → specificity diminishes. The error rate can go either way, depending on this tradeoff. In contrast, raising the classification threshold to 70% has the opposite effect, that is sensitivity declines (i.e., fewer observations are classified as positive) and specificity improves (i.e., more observations are classified as negative).
  2. It all depends on the business importance of positive vs. negative classification. What is better, to convict more guilty or to let go more innocents? Is it more important to make correct positive medical diagnoses or more correct negative diagnoses? If making correct positive predictions is better, then lower the classification threshold. If making correct negative predictions is better, then raise the classification threshold. But be careful, if you lower or raise the thresholds to extreme (e.g., 5% or 95%) the error rate will increase.
  3. If interpretation is the analysis goal, the Logistic regression is superior. If predictive accuracy is the goals, you can run all 3 models and select one based on cross-validation testing. Generally speaking, LDA and QDA require the predictors to be normally distributed, because the predictions are based on estimating probabilities using a normal distribution curve, whereas Logistic regression does not require predictors to be normally distributed with sufficiently large samples. But once again, if you are trying various models you should always compute cross-validation statistics to evaluate them.